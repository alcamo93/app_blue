...
const TABLES = [
  { name: 'pag120102fotos', dir: 'pag120102fotos' },
];
...
async function main() {
  ...
  for (const { name: table, dir } of TABLES) {
    const [rows] = await db.query(
      `SELECT Loc, Tabla AS template_raw, photo_1_name, photo_2_name FROM \`${table}\``
    );

    for (const row of rows) {
      const { Loc, template_raw, photo_1_name, photo_2_name } = row;
      ...
      for (const [question, suffix, photoName] of [
        ...
        try {
          ...
          console.log(`✅ ${dir}/${Loc}_${suffix}_${template}.jpg`);
        } catch (err) {
